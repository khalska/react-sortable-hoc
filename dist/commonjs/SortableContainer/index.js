...
function sortableContainer(WrappedComponent) {
  ...
  return _temp = _class = function (_Component) {
    ...
    function _class(props) {
      ...
      _this.handleSortEnd = function (e) {
        ...
        for (var i = 0, len = nodes.length; i < len; i++) {
          ...
          if (_this.props.translatableSelector) {
            var els = el.querySelectorAll(_this.props.translatableSelector);
            for (var j = 0, jlen = els.length; j < jlen; j++) {
              els[j].style[_utils.vendorPrefix + 'Transform'] = '';
              els[j].style[_utils.vendorPrefix + 'TransitionDuration'] = '';
            }
          } else {
        ...
        for (var i = 0, len = nodes.length; i < len; i++) {
          ...
          if (transitionDuration) {
            if (this.props.translatableSelector) {
              var els = node.querySelectorAll(this.props.translatableSelector);
              for (var j = 0, jlen = els.length; j < jlen; j++) {
                els[j].style[_utils.vendorPrefix + 'TransitionDuration'] = transitionDuration + 'ms';
              }
            } else {
          ...
          if (this.props.translatableSelector) {
            var _els = node.querySelectorAll(this.props.translatableSelector);
            for (var _j = 0, _jlen = _els.length; _j < _jlen; _j++) {
              _els[_j].style[_utils.vendorPrefix + 'Transform'] = 'translate3d(' + translate.x + 'px,' + translate.y + 'px,0)';
            }
          } else {
    ...
    translatableSelector: _propTypes2.default.string,
